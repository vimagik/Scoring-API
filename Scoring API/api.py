...
def clients_interests_handler(request: MethodRequest, ctx: dict) -> tuple:
    ...
    if clients_interest.error is not None:
        ...
        code = 200
        response = {i: scoring.get_interests() for i in clients_interest.client_ids}
    return code, response
...
def method_handler(request: dict, ctx: dict, store) -> tuple:
    ...
    if request.error is not None:
        ...
        code, response = clients_interests_handler(request, ctx)
